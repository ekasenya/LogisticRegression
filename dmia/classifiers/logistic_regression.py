...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            ind = np.random.choice(X.shape[0], size=batch_size)
            X_batch = X[ind]
            y_batch = y[ind]
    ...
    def predict(self, X):
        ...
        ###########################################################################
        y_proba = self.predict_proba(X, True)
        y_pred = np.where(y_proba[0] > y_proba[1], 1, 0)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_pred
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        loss = -np.sum(y_batch * np.log(y_proba[0]) + (1 - y_batch) * np.log(1 - y_proba[0])) / n
        dw = (X_batch.T.dot(y_proba[0] - y_batch)) / n

        # Add regularization to the loss and gradient.
        # Note that you have to exclude bias term in regularization.
        loss += reg * np.sum(self.w[:-1] ** 2) / n
        dw[:-1] += reg * np.sign(dw[:-1]) / n
